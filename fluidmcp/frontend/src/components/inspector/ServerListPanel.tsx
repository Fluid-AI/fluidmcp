...
import { type SavedRequest, loadSavedRequests, deleteSavedRequest, renameSavedRequest } from "@/lib/saved-requests";
...
interface ServerListPanelProps {
  ...
  renameSavedRequest: typeof renameSavedRequest;
...
export const ServerListPanel: React.FC<ServerListPanelProps> = ({
  ...
  setRenamingId,
  setRenameTitle,
  renameSavedRequest: renameSavedRequestFn,
  onDisconnect,
  onReconnect,
  onRemove,
  onAddServer,
                        ...
                        {savedRequests.length === 0 ? (
                          ...
                          <div
                            ...
                            style={{
                              ...
                              background: "rgba(0,0,0,0.2)", cursor: renamingId === req.id ? "default" : "pointer",
                            }}
                            onClick={() => {
                            ...
                            {renamingId === req.id ? (
                              <div style={{ display: "flex", gap: "0.3rem", alignItems: "center" }} onClick={e => e.stopPropagation()}>
                                <input
                                  ...
                                  onKeyDown={e => {
                                    if (e.key === "Enter" && renameTitle.trim() && selectedServer) {
                                      renameSavedRequestFn(selectedServer.url, req.id, renameTitle.trim());
                                ...
                                <button
                                  onClick={() => {
                                    if (renameTitle.trim() && selectedServer) {
                                      renameSavedRequestFn(selectedServer.url, req.id, renameTitle.trim());
                                      setSavedRequests(loadSavedRequests(selectedServer.url));
                                    }
                                    setRenamingId(null);
                                  ...
                                  style={{
                                    ...
                                    color: "rgba(165,180,252,0.9)", cursor: "pointer", flexShrink: 0,
                                  }}
                                >✓</button>
                                <button
