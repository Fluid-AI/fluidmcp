...
class Auth0Config(BaseModel):
    ...
    def from_env_or_file(cls, file_path: Optional[str] = None) -> 'Auth0Config':
        ...
        if os.getenv('AUTH0_DOMAIN'):
            env_overrides['domain'] = os.getenv('AUTH0_DOMAIN')
        if os.getenv('AUTH0_CLIENT_ID'):
            env_overrides['client_id'] = os.getenv('AUTH0_CLIENT_ID')
        if os.getenv('AUTH0_CLIENT_SECRET'):
            env_overrides['client_secret'] = os.getenv('AUTH0_CLIENT_SECRET')
        if os.getenv('AUTH0_CALLBACK_URL'):
            env_overrides['callback_url'] = os.getenv('AUTH0_CALLBACK_URL')
        if os.getenv('AUTH0_AUDIENCE'):
            env_overrides['audience'] = os.getenv('AUTH0_AUDIENCE')
    ...
    def validate_required(self) -> None:
        ...
        if not self.domain:
            missing_fields.append("domain (AUTH0_DOMAIN or auth0-config.json)")
        if not self.client_id:
            missing_fields.append("client_id (AUTH0_CLIENT_ID or auth0-config.json)")
        if not self.client_secret:
            missing_fields.append("client_secret (AUTH0_CLIENT_SECRET or auth0-config.json)")
